...
export default {
  ...
  methods: {
    ...
    async submitEdit() {
      ...
      try {
        const token = localStorage.getItem('jwt_token');
        const res = await fetch(`http://146.190.93.28/api/transactions/${this.editTransaction.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
    ...
    async confirmDelete(transactionId) {
      ...
      try {
        const token = localStorage.getItem('jwt_token');
        const res = await fetch(`http://146.190.93.28/api/transactions/${transactionId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
